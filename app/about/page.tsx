...
const differences = [
  { icon: LayoutPanelTop, title: "Free Marketing Kit With Every Package", copy: "Every booking automatically includes 9 social media reels, 3 branded property websites, 3 print-ready flyers, and 2 animated slideshows — delivered alongside your photos the next morning." },
...
function StorySection() {
  return <section className="about-story-section" aria-labelledby="about-story-heading"><div className="container about-story-grid"><div className="about-copy"><span className="section-label">Our Story</span><h2 id="about-story-heading">We Built Photos4RealEstate to Give Calgary Realtors More Than Just Photos</h2><p className="lead speakable-intro">Photos4RealEstate was founded on a straightforward observation: Calgary realtors were spending significant time and money pulling together the visual assets needed for a modern listing — photography from one provider, RMS measurements from another, a virtual tour from a third, then hours assembling flyers, reels, and websites manually.</p><p>We built Photos4RealEstate to solve that. One booking. One visit. One delivery the next morning — professional photography, RECA-compliant RMS measurements, iGUIDE 3D virtual tour, floor plans, drone footage, and a complete marketing kit with 9 reels, 3 property websites, 3 flyers, and 2 slideshows.</p><p>We are a small, specialist team — Iryna and Vlad — and that is intentional. Every booking is handled personally by us. The consistency of quality, the care in every shot, and the on-time delivery come from a team that takes every listing personally.</p></div><div className="about-story-visual" aria-label="Photos 4 Real Estate work examples"><div className="about-story-img about-story-img-main"><Image src={images.storyMain} alt="Real estate photography by Photos4RealEstate in Calgary" fill sizes="(max-width: 960px) 100vw, 50vw" /></div><div className="about-story-img"><Image src={images.storyDrone} alt="Kitchen real estate photography by Photos4RealEstate in Calgary" fill sizes="(max-width: 960px) 50vw, 25vw" /></div><div className="about-story-img"><Image src={images.storyFloorPlan} alt="Standard iGUIDE floor plan for a Calgary MLS listing" fill sizes="(max-width: 960px) 50vw, 25vw" /></div></div></div></section>;
}
...
function CompareSection() {
  return <CardGrid sectionClass="about-diff-section" label="How We Compare" heading="What Sets Photos4RealEstate Apart from Other Calgary Photography Companies" intro="Many companies offer photos. Some offer virtual tours. We built something different — a complete listing system where everything works together." cards={differences} dark><div className="about-diff-link"><Link href="/services/real-estate-photography-in-calgary">Compare our complete Calgary real estate photography service<span className="sr-only"> with standard listing photography companies</span></Link></div></CardGrid>;
}
