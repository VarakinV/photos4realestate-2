...
import { CalendarDays, CheckCircle2, Camera, Video, Ruler, Box, Sunset, Copyright } from "lucide-react";
...
import { blogPosts, formatBlogDate, getBlogOpenGraphImage, getBlogPost, getPostCategories, type BlogSection } from "@/lib/blog";
import { siteConfig } from "@/lib/site";

const blogIconMap: Record<string, React.ComponentType<{ size?: number; className?: string }>> = {
  Camera,
  Video,
  Ruler,
  Box,
...
function BlogRichBlocks({ section }: { section: BlogSection }) {
  return (
    <>
      ...
      {section.embed ? <figure className="blog-embed-card"><div className="blog-embed-frame" style={{ paddingBottom: section.embed.aspectRatio ?? "57%" }}><iframe src={section.embed.src} title={section.embed.title} loading="lazy" scrolling="no" allowFullScreen referrerPolicy="strict-origin-when-cross-origin" /></div>{section.embed.caption || section.embed.href ? <figcaption><span>{section.embed.caption}</span>{section.embed.href && section.embed.linkLabel ? <a href={section.embed.href} target="_blank" rel="noopener noreferrer">{section.embed.linkLabel}</a> : null}</figcaption> : null}</figure> : null}
      ...
      {section.beforeAfterComparisons?.length ? <div className="blog-before-after-list">{section.beforeAfterComparisons.map((comparison) => <figure className="blog-before-after-card" key={comparison.title}><div className="blog-before-after-slider"><BeforeAfter beforeSrc={comparison.beforeSrc} afterSrc={comparison.afterSrc} beforeAlt={comparison.beforeAlt} afterAlt={comparison.afterAlt} beforeLabel={comparison.beforeLabel ?? "Before"} afterLabel={comparison.afterLabel ?? "After"} /></div><figcaption><strong>{comparison.title}</strong><span>{comparison.caption}</span></figcaption></figure>)}</div> : null}
      {section.serviceCards?.length ? <div className="blog-service-cards">{section.serviceCards.map((card) => { const IconComponent = blogIconMap[card.icon] ?? Camera; return <Link key={card.label} href={card.href} className="blog-service-card"><div className="blog-service-icon"><IconComponent size={24} aria-hidden="true" /></div><strong>{card.label}</strong><span>{card.description}</span></Link>; })}</div> : null}
      {section.highlightBlock ? <div className="blog-highlight-block">{(() => { const IconComponent = blogIconMap[section.highlightBlock!.icon] ?? Copyright; return <IconComponent size={24} aria-hidden="true" />; })()}<div>{section.highlightBlock.paragraphs.map((p) => <p key={p}>{p}</p>)}</div></div> : null}
    </>
  );
}
      ...
      <section className="blog-post-section">
        <div className="container blog-layout">
          <article className="blog-post-article">
            ...
            <section className="blog-related-services" aria-labelledby="related-services-heading"><h2 id="related-services-heading">Related Photos 4 Real Estate services</h2><ul>{post.relatedServices.map((service) => <li key={`${service.href}-${service.label}`}><Link href={service.href}>{service.label}</Link></li>)}</ul></section>
          </article>
          <BlogSidebar />
        </div>
      </section>
